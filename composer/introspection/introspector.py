...
class Introspector(Node):
    ...
    def kill(self, name: str, pid: int):
        """
        ...
        try:
            result = subprocess.run(
                ['kill', str(pid)], check=True, capture_output=True, text=True)
            if result.returncode == 0:
                self.get_logger().info(
                    f'Successfully killed {name} with PID: {pid}')
            else:
                self.get_logger().error(
                    f'Failed to kill {name} with PID: {pid}. Return code: {result.returncode}')
        except subprocess.CalledProcessError as e:
            self.get_logger().error(
                f'Kill was not successful for {name}. Error: {e.stderr}')
        except Exception as e:
            self.get_logger().error(
                f'Unexpected error while trying to kill {name}. Exception message: {e}')
