...
class MutoDefaultLaunchPlugin(Node):
    ...
    def _bootstrap(self):
        ...
        self.twin = twin.Twin(node='MutoLaunchPlugin',
                              config=self.muto)
        self.device = edge.EdgeDevice(self, twin=self.twin)

    def handle_apply(self, req: PlanManifest, res: PlanManifest):
        """Handles requests to apply changes to a stack."""
        return self._handle_stack_operation(req, res, self.device.apply)

    def handle_kill(self, req: PlanManifest, res: PlanManifest):
        """Handles requests to kill a stack."""
        return self._handle_stack_operation(req, res, self.device.kill)

    def handle_start(self, req: PlanManifest, res: PlanManifest):
        """Handles requests to start a stack."""
        return self._handle_stack_operation(req, res, self.device.activate)

    def _handle_stack_operation(self, req: PlanManifest, res: PlanManifest, operation_method: Callable):
        """
    ...
    def _success_response(self, planned: PlanManifest):
    ...
    def _error_response(self, error_message: str):
        ...
        return PlanManifest(
            result=PluginResponse(
                result_code=1000, error_message=error_message, error_description=error_message)
        )
